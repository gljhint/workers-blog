...
export default function EditPage({ params }: PageEditProps) {
  ...
  useEffect(() => {
    if (isAuthenticated && pageId !== null && !isNaN(pageId)) {
      loadPage();
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (result.success) {
        ...
        setError(result.error || '更新页面失败');
      }
    } catch (error) {
      console.error('更新页面失败:', error);
      setError('更新页面失败，请稍后重试');
    } finally {
      setSubmitting(false);
    }
  };
